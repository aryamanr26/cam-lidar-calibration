...
class BagDataExtractor(Node):
    def __init__(self):
        super().__init__('all_data_extractor')
        
    def extract_all_data(self, bag_file, output_dir="extracted_data"):
        """Extract all images and point clouds from the bag file"""
        
        camera_topics = {
            '/arenacam1/images': 'cam1',
            '/arenacam2/images': 'cam2',
            '/arenacam3/images': 'cam3',
            '/arenacam4/images': 'cam4',
            '/arenacam5/images': 'cam5',
            '/arenacam6/images': 'cam6',
        }

        # Create organized output directories
        directory_name = "cam_lidar_data" # or "cam4_cal_data" or "cam6_cal_data"

        base_dir = os.path.join(output_dir, directory_name)

        image_dirs = {}
        for topic, cam_name in camera_topics.items():
            image_dirs[topic] = os.path.join(base_dir, "images", cam_name)
            os.makedirs(image_dirs[topic], exist_ok=True)
        ...
        while reader.has_next():
            ...
            # topic_name = '/arenacam5/images' # or '/arenacam4/images' or '/arenacam2/images'
            if topic in camera_topics and topic_type == 'sensor_msgs/msg/Image':
                try:
                    ...
                    if cv_image is not None:
                        ...
                        cam_dir = image_dirs[topic]
                        filename = f"{camera_topics[topic]}_{image_count:06d}_{dt.strftime('%H%M%S_%f')[:-3]}.png"
        ...
        self.create_summary_file(base_dir, image_count, pcd_counts, camera_topics)
    ...
    def create_summary_file(self, base_dir, image_count, pcd_counts, camera_topics):
        """Create a summary file with extraction details"""
        summary_file = os.path.join(base_dir, "extraction_summary.txt")
        
        with open(summary_file, 'w') as f:
            f.write("Multi-Camera & LiDAR Data Extraction Summary\n")
            f.write("="*50 + "\n\n")
            f.write(f"Extraction Date: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n\n")
            
            f.write("Extracted Data:\n")
            f.write(f"  Total Images: {image_count} files\n")
            f.write(f"  Camera Topics:\n")
            for topic, cam_name in camera_topics.items():
                f.write(f"    - {cam_name}: saved in images/{cam_name}/\n")
            ...
            f.write(f"  {os.path.relpath(base_dir)}\n")
            f.write("  ├── images/\n")
            for cam_name in camera_topics.values():
                f.write(f"  │   └── {cam_name}/image_XXXXXX_HHMMSS_mmm.png\n")
            f.write("  ├── pointclouds/\n")
            f.write("  │   ├── front/\n")
            f.write("  │   ├── back/\n")
            f.write("  │   ├── left/\n")
